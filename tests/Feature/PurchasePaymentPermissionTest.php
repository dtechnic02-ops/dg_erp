...
class PurchasePaymentPermissionTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        foreach ([
            'purchase_payments', 'purchase_invoices', 'accounts', 'suppliers',
        ...
        Schema::create('accounts', fn (Blueprint $t) => [$t->id(), $t->unsignedBigInteger('company_id'), $t->string('account_type'), $t->string('account_name'), $t->decimal('current_balance', 20, 4)->default(0), $t->string('status')->default('active'), $t->timestamps()]);
        ...
        DB::table('accounts')->insert(['id' => 1, 'company_id' => 1, 'account_type' => 'Cash', 'account_name' => 'Cash', 'current_balance' => 1000, 'status' => 'active']);
    ...
    public function test_unauthorized_staff_is_rejected_by_every_purchase_payment_route(): void
    {
        [$payment, $invoice] = $this->paymentContext(1);
        $staff = User::find(2);

        foreach ($this->routes($payment, $invoice) as [$method, $name, $params, $data]) {
            $this->actingAs($staff)->{$method}(route($name, $params), $data)->assertForbidden();
        }
    }

    public function test_company_admin_and_authorized_staff_can_access_purchase_payment_routes(): void
    {
        [$payment, $invoice] = $this->paymentContext(1);
    ...
    public function test_cross_company_purchase_payment_access_remains_blocked_for_admin(): void
    {
        [$foreignPayment] = $this->paymentContext(2);
        $admin = User::find(1);

        $this->actingAs($admin)->get(route('company.purchase-payments.show', $foreignPayment->id))->assertNotFound();
        $this->actingAs($admin)->get(route('company.purchase-payments.edit', $foreignPayment->id))->assertNotFound();

        $this->actingAs($admin)
            ->post(route('company.purchase-payments.update', $foreignPayment->id), $this->updatePayload())
            ->assertRedirect()
            ->assertSessionHas('error');

        $this->actingAs($admin)
            ->post(route('company.purchase-payments.cancel', $foreignPayment->id), $this->cancelPayload())
            ->assertRedirect()
            ->assertSessionHas('error');

        $this->assertSame(1, PurchasePayment::find($foreignPayment->id)->status);
    }

    private function routes(PurchasePayment $payment, PurchaseInvoice $invoice): array
    {
        return [
            ['get', 'company.purchase-payments.index', [], []],
            ['get', 'company.purchase-payments.create', [$invoice->id], []],
            ['post', 'company.purchase-payments.store', [], $this->storePayload($invoice)],
            ['get', 'company.purchase-payments.show', [$payment->id], []],
            ['get', 'company.purchase-payments.edit', [$payment->id], []],
            ['post', 'company.purchase-payments.update', [$payment->id], $this->updatePayload()],
            ['get', 'company.purchase-payments.print-list', [], []],
            ['get', 'company.purchase-payments.print', [$payment->id], []],
            ['post', 'company.purchase-payments.cancel', [$payment->id], $this->cancelPayload()],
        ];
    }

    private function paymentContext(int $companyId): array
    {
        ...
        $payment = PurchasePayment::create([
            ...
            'supplier_id' => $supplierId,
            'account_id' => 1,
            'payment_no' => 'PP-' . $companyId . '-1-00001',
    ...
    private function storePayload(PurchaseInvoice $invoice): array
    {
        return [
            'purchase_invoice_id' => $invoice->id,
            'account_id' => 1,
