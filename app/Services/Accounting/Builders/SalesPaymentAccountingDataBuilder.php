...
class SalesPaymentAccountingDataBuilder
{
    public function build(SalesPayment $payment): array
    {
        if (! $payment->exists || (int) $payment->status !== SalesPayment::STATUS_ACTIVE) {
            throw new RuntimeException('Only an active persisted sales payment can be posted to accounting.');
        }
        ...
        if ($this->isZero($amount)) {
            throw new RuntimeException('The persisted sales payment amount must be greater than zero.');
        }
        ...
        if (! $financialYear || $paymentDate < $financialYear->start_date || $paymentDate > $financialYear->end_date) {
            throw new RuntimeException('The sales payment date does not belong to its company financial year.');
        }

        $invoice = SalesInvoice::query()
            ->where('company_id', $companyId)
            ->find($this->positiveInteger($payment->sales_invoice_id, 'sales_invoice_id'));

        if (! $invoice || (int) $invoice->status !== 1 || (int) $invoice->financial_year_id !== $financialYearId || (int) $invoice->customer_id !== $customerId) {
            throw new RuntimeException('The sales payment, invoice, customer, company, and financial year must belong together.');
        }

        $accountTransaction = AccountTransaction::query()
            ->where('company_id', $companyId)
        ...
        if ($accountTransaction->count() !== 1) {
            throw new RuntimeException('The sales payment must have exactly one active account transaction.');
        }
        ...
        if ((int) $payment->account_id !== $accountId || (int) $accountTransaction->financial_year_id !== $financialYearId || ! $this->isZero($this->amount($accountTransaction->credit, 'account transaction credit')) || ! $this->same($this->amount($accountTransaction->debit, 'account transaction debit'), $amount)) {
            throw new RuntimeException('The sales payment account transaction does not match the persisted payment.');
        }
        ...
        if (! $account || ! in_array($account->account_type, ['Cash', 'Bank', 'ATM', 'Wallet'], true)) {
            throw new RuntimeException('The sales payment operational account is invalid.');
        }
        ...
        if ($customerTransactions->count() !== 1) {
            throw new RuntimeException('The sales payment must have exactly one active customer transaction.');
        }

        $customerTransaction = $customerTransactions->first();

        if ((int) $customerTransaction->customer_id !== $customerId || (int) $customerTransaction->financial_year_id !== $financialYearId || ! $this->isZero($this->amount($customerTransaction->debit, 'customer transaction debit')) || ! $this->same($this->amount($customerTransaction->credit, 'customer transaction credit'), $amount)) {
            throw new RuntimeException('The sales payment customer transaction does not match the persisted payment.');
        }
