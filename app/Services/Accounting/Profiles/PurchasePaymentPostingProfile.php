...
class PurchasePaymentPostingProfile
{
    public function build(array $data): array
    {
        ...
        if ($amount === '0.0000') {
            throw new RuntimeException('A purchase payment accounting amount must be greater than zero.');
        }

        $operationalCode = match ($accountType) {
            'Cash' => 'CASH_IN_HAND',
            'Bank', 'ATM', 'Wallet' => 'BANK_ACCOUNTS',
            default => throw new RuntimeException('Unsupported operational account type for purchase payment accounting.'),
        };
        ...
        return [
            ...
            'source_type_aliases' => [PurchasePayment::class],
            'source_id' => $paymentId,
            'source_event' => 'created',
            'source_key' => 'purchase_payment:' . $paymentId . ':created',
            'description' => 'Purchase payment - ' . $paymentNumber,
            'posted_by' => $data['created_by'] ?? null,
            'lines' => [
                [
                    'chart_account_system_code' => 'ACCOUNTS_PAYABLE',
                    'operational_account_id' => null,
                    'description' => 'Purchase payable settlement - ' . $paymentNumber,
                    'debit' => $amount,
                    'credit' => '0.0000',
                ...
                [
                    ...
                    'description' => 'Purchase payment disbursement - ' . $paymentNumber,
                    'debit' => '0.0000',
                    'credit' => $amount,
                    'subledger_type' => null,
                    'subledger_id' => null,
