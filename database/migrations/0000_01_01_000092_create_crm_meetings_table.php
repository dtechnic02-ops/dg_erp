...
return new class extends Migration
{
    public function up(): void
    {
        if (DB::getDriverName() === 'mysql') {
            DB::statement('CREATE TABLE `crm_meetings` (
  ...
  CONSTRAINT `crm_meetings_updated_by_foreign` FOREIGN KEY (`updated_by`) REFERENCES `users` (`id`) ON DELETE SET NULL
        ...
        Schema::create('crm_meetings', function (Blueprint $table): void {
            ...
            $table->index(array (
  0 => 'archived_by',
), 'crm_meetings_archived_by_foreign');
            $table->index(array (
  0 => 'assigned_employee_id',
), 'crm_meetings_assigned_employee_id_foreign');
            $table->index(array (
  0 => 'cancelled_by',
), 'crm_meetings_cancelled_by_foreign');
            $table->index(array (
  0 => 'company_id',
  1 => 'meeting_date',
), 'crm_meetings_company_date_index');
            $table->index(array (
  0 => 'company_id',
  1 => 'financial_year_id',
), 'crm_meetings_company_fy_index');
            $table->unique(array (
  0 => 'company_id',
  1 => 'activity_no',
), 'crm_meetings_company_no_unique');
            $table->index(array (
  0 => 'created_by',
), 'crm_meetings_created_by_foreign');
            $table->index(array (
  0 => 'crm_lead_id',
), 'crm_meetings_crm_lead_id_foreign');
            $table->index(array (
  0 => 'crm_opportunity_id',
), 'crm_meetings_crm_opportunity_id_foreign');
            $table->index(array (
  0 => 'financial_year_id',
), 'crm_meetings_financial_year_id_foreign');
            $table->index(array (
  0 => 'updated_by',
), 'crm_meetings_updated_by_foreign');
            $table->foreign(array (
  0 => 'archived_by',
), 'crm_meetings_archived_by_foreign')->references(array (
  0 => 'id',
))->on('users')->onDelete('set null')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'assigned_employee_id',
), 'crm_meetings_assigned_employee_id_foreign')->references(array (
  0 => 'id',
))->on('employee_accounts')->onDelete('restrict')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'cancelled_by',
), 'crm_meetings_cancelled_by_foreign')->references(array (
  0 => 'id',
))->on('users')->onDelete('set null')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'company_id',
), 'crm_meetings_company_id_foreign')->references(array (
  0 => 'id',
))->on('companies')->onDelete('cascade')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'created_by',
), 'crm_meetings_created_by_foreign')->references(array (
  0 => 'id',
))->on('users')->onDelete('set null')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'crm_lead_id',
), 'crm_meetings_crm_lead_id_foreign')->references(array (
  0 => 'id',
))->on('crm_leads')->onDelete('set null')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'crm_opportunity_id',
), 'crm_meetings_crm_opportunity_id_foreign')->references(array (
  0 => 'id',
))->on('crm_opportunities')->onDelete('set null')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'financial_year_id',
), 'crm_meetings_financial_year_id_foreign')->references(array (
  0 => 'id',
))->on('financial_years')->onDelete('set null')->onUpdate('restrict');
            $table->foreign(array (
  0 => 'updated_by',
), 'crm_meetings_updated_by_foreign')->references(array (
  0 => 'id',
))->on('users')->onDelete('set null')->onUpdate('restrict');
        });
    }
