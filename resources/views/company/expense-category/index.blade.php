...
@php
    $user = auth()->user();
    $canManage = $user && ($user->role_id == 2 || $user->hasPermission('manage_expense_categories'));
@endphp
        ...
        <div class="container-fluid">
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                <div class="flex-fill d-flex justify-content-end align-items-center gap-2 flex-wrap flex-md-nowrap">
                    <nav class="btn-group flex-wrap" aria-label="Expense category toolbar">
                        <a href="{{ route('company.expense.index') }}" class="btn btn-sm btn-outline-secondary dg-btn">Expense List</a>
                        @if ($canManage)
                            <a href="{{ route('company.expense-category.create') }}" class="btn btn-sm btn-success dg-btn">Add Category</a>
                        @endif
                    </nav>
                </div>
            </div>
            ...
            <section class="dg-section dg-filter">
                <article class="card dg-card">
                    ...
                    <div class="card-body dg-card-body dg-filter-card-body">
                        <form method="GET" action="{{ route('company.expense-category.index') }}" class="dg-filter-form">
                            <div class="dg-filter-grid">
                                ...
                                <div class="dg-filter-actions">
                                    ...
                                    <a href="{{ route('company.expense-category.index') }}" class="btn btn-outline-secondary dg-btn">Reset</a>
                                </div>
                            </div>
                        </form>
                    </div>
                                            ...
                                            <td class="dg-action-col">
                                                @if ($canManage)
                                                    <div class="dg-action-group" role="group" aria-label="Category actions for {{ $category->name }}">
                                                        <a href="{{ route('company.expense-category.edit', $category->id) }}" class="btn btn-sm btn-outline-primary dg-action-btn">Edit</a>
                                                        <form method="POST" action="{{ route('company.expense-category.delete', $category->id) }}" class="d-inline" onsubmit="return confirm('Delete this category?')">
                                                            @csrf
