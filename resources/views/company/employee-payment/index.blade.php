...
<div class="dg-page">
        ...
        <div class="container-fluid">
            <div class="d-flex flex-nowrap align-items-center gap-2">
                ...
                <div class="flex-fill d-flex justify-content-end align-items-center gap-2 flex-wrap flex-md-nowrap">
                    <nav class="btn-group" aria-label="Salary payment toolbar">
                        <a href="{{ route('company.dashboard') }}" class="btn btn-outline-secondary dg-btn">Dashboard</a>
                        @if (userCan('salary.payment.create'))
                            <a href="{{ route('company.salary-sheets.index') }}" class="btn btn-primary dg-btn">New Payment</a>
                        @endif
                        <a href="{{ route('company.employee-payment.index') }}" class="btn btn-outline-secondary dg-btn">Refresh</a>
                        <a href="{{ route('company.employee-payment.print-list', request()->query()) }}" target="_blank" class="btn btn-outline-secondary dg-btn">Print List</a>
                        @if (userCan('salary.view'))
                            <a href="{{ route('company.salary-sheets.index') }}" class="btn btn-outline-secondary dg-btn">Salary Sheets</a>
                        @endif
    ...
    <main class="dg-container">
        <div class="container-fluid">
            ...
            <section class="dg-section dg-filter @if (request('print')) d-print-none @endif">
                <article class="card dg-card">
                    ...
                    <div class="card-body dg-card-body dg-filter-card-body">
                        <form method="GET" action="{{ route('company.employee-payment.index') }}" class="dg-filter-form">
                            <div class="dg-filter-grid">
                                ...
                                <div class="dg-filter-field dg-filter-field-invoice">
                                    <label for="voucher_no" class="dg-filter-label">Voucher No</label>
                                    <input type="text" name="voucher_no" id="voucher_no" class="form-control dg-input dg-filter-control" value="{{ request('voucher_no') }}" placeholder="Voucher No">
                                </div>

                                <div class="dg-filter-field dg-filter-field-invoice">
                                    <label for="salary_month" class="dg-filter-label">Salary Month</label>
                                    <input type="month" name="salary_month" id="salary_month" class="form-control dg-input dg-filter-control" value="{{ request('salary_month') }}">
                                </div>
                                ...
                                <div class="dg-filter-field dg-filter-field-actions">
                                    ...
                                    <div class="dg-filter-actions">
                                        ...
                                        <a href="{{ route('company.employee-payment.index') }}" class="btn btn-outline-secondary dg-btn dg-filter-btn">Reset</a>
                                    </div>
                                </div>
            ...
            <section class="dg-section" id="dgSalaryPaymentList">
                <article class="card dg-card dg-print">
                    <header class="card-header dg-card-header dg-list-card-header">
                        <h2 class="dg-list-card-title">Salary Payment List</h2>

                        <form method="GET" action="{{ route('company.employee-payment.index') }}" class="dg-list-per-page @if (request('print')) d-print-none @endif">
                            ...
                            <input type="hidden" name="voucher_no" value="{{ request('voucher_no') }}">
                                ...
                                <tbody class="dg-body">
                                    @forelse ($payments as $payment)
                                        <tr class="dg-row">
                                            ...
                                            <td class="dg-col-num dg-col-paid">{{ number_format($payment->amount, 2) }}</td>
                                            ...
                                            <td class="dg-action-col d-print-none">
                                                <div class="dg-action-group" role="group" aria-label="Payment actions for {{ $payment->voucher_no }}">
                                                    <a href="{{ route('company.employee-payment.show', $payment->id) }}" class="btn btn-sm btn-outline-primary dg-action-btn">View</a>
                                                </div>
                                            </td>
                                        </tr>
                                    @empty
