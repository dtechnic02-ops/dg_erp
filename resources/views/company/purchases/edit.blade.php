...
<div class="dg-page">
    ...
    <header class="dg-toolbar">
        <div class="container-fluid">
            <div class="row align-items-center g-2">
                <div class="col">
                    <h1 class="h4 mb-0">Edit Purchase Invoice</h1>
                </div>
                <div class="col-auto">
                    <nav class="btn-group" aria-label="Purchase toolbar">
                        ...
                        <a href="{{ route('company.purchases.show', $invoice->id) }}" class="btn btn-outline-secondary dg-btn">View Invoice</a>
                    </nav>
                </div>
            </div>
        </div>
    ...
    <main class="dg-container">
        <div class="container-fluid">
            ...
            <form id="dgForm" method="POST" action="{{ route('company.purchases.update', $invoice->id) }}">
                @csrf
                @method('PUT')
                        ...
                        <div class="card-body dg-card-body">
                            <div class="row g-3 align-items-start">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="text" id="supplier_name" class="form-control dg-input" value="{{ $invoice->supplier->name ?? '-' }}" readonly>
                                    <small class="form-text text-muted dg-note">Supplier Balance: {{ number_format($invoice->supplier->current_balance ?? 0, 2) }}</small>
                                </div>
                                    ...
                                    <tbody class="dg-body">
                                        @foreach ($invoice->items as $index => $item)
                                            <tr class="dg-row">
                                                ...
                                                <td>
                                                    <input type="text" class="form-control dg-input" value="{{ $item->item_type === 'product' ? ($item->product->unit?->short_name ?? $item->product->unit?->name ?? '-') : ($item->item_type === 'service' ? 'Service' : '-') }}" readonly aria-label="Unit">
                                                </td>

                                                <td>
                                                    <input type="text" class="form-control dg-input text-end" value="{{ number_format($item->unit_price, 2) }}" readonly aria-label="Unit Cost">
                                                </td>
                                                ...
                                                <td>
                                                    <input type="text" class="form-control dg-input text-end" value="{{ number_format($item->total_price, 2) }}" readonly aria-label="Line Total">
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
