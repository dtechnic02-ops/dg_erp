        ...
        <div class="container-fluid">
            <div class="d-flex flex-nowrap align-items-center gap-2">
                ...
                <div class="flex-fill d-flex justify-content-end align-items-center gap-2 flex-nowrap">
                    @if ($canCreate)
                        <a href="{{ route('company.journal.create') }}" class="btn btn-success dg-btn">New Journal</a>
                    @endif
                </div>
            </div>
        </div>
            ...
            <section class="dg-section dg-filter">
                <article class="card dg-card">
                    ...
                    <div class="card-body dg-card-body dg-filter-card-body">
                        <form method="GET" action="{{ route('company.journal.index') }}" class="dg-filter-form">
                            <div class="dg-filter-grid">
                                <div class="dg-filter-field">
                                    <label for="search" class="dg-filter-label">Search</label>
                                    <input type="text" name="search" id="search" class="form-control dg-input dg-filter-control" value="{{ request('search') }}" placeholder="Journal No / Reference / Narration">
                                </div>
                                ...
                                <div class="dg-filter-actions">
                                    ...
                                    <a href="{{ route('company.journal.index') }}" class="btn btn-outline-secondary dg-btn">Reset</a>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <section class="dg-section">
                <article class="card dg-card">
                    <header class="card-header dg-card-header dg-list-card-header">
                        <h2 class="dg-list-card-title">Journal List</h2>

                        <form method="GET" action="{{ route('company.journal.index') }}" class="dg-list-per-page">
                    ...
                    <div class="card-body dg-card-body dg-list-card-body">
                        ...
                        <div class="dg-list-footer">
                            <p class="dg-list-meta">
                                Showing {{ $journals->firstItem() ?? 0 }} to {{ $journals->lastItem() ?? 0 }} of {{ $journals->total() }} records
                            </p>
                            <div class="dg-pagination">
                                {{ $journals->withQueryString()->onEachSide(1)->links('pagination::bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </article>
