...
@section('title', 'Loan Saving Ledger')

@section('content')

@php
    $canView = userCan('view_loan_saving_ledger');
...
<div class="dg-page loan-saving-ledger-page @if (request('print')) dg-print-page @endif">
        ...
        <div class="container-fluid">
            <div class="d-flex flex-nowrap align-items-center gap-2">
                ...
                <div class="flex-shrink-0">
                    <h1 class="h4 mb-0">Loan Saving Ledger</h1>
                </div>

                <div class="flex-fill d-flex justify-content-end align-items-center gap-2 flex-wrap flex-md-nowrap">
                    <nav class="btn-group flex-wrap" aria-label="Loan saving ledger toolbar">
                        ...
                        @endif
                        <a href="{{ route('company.loan-saving-ledger.index') }}" class="btn btn-outline-secondary dg-btn">Refresh</a>
                        @if ($canPrint)
                            <button type="button" class="btn btn-outline-secondary dg-btn" onclick="window.print()">Print</button>
                        @endif
                        <button type="button" class="btn btn-outline-secondary dg-btn" id="loanSavingLedgerExportBtn">Export</button>
                    </nav>
                </div>
                ...
                <div class="row g-2">
                        ...
                        <article class="card dg-card h-100">
                            <div class="card-body dg-card-body py-2">
                                <div class="dg-summary-item mb-0 border-0 p-0">
                                    <span class="dg-label d-block mb-1">Total Entries</span>
                                    <span class="fw-bold fs-5">{{ number_format($totalEntries) }}</span>
                                </div>
                            </div>
                        </article>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <article class="card dg-card h-100">
                            <div class="card-body dg-card-body py-2">
                                <div class="dg-summary-item mb-0 border-0 p-0">
                                    <span class="dg-label d-block mb-1">Total Deposit</span>
                                    <span class="fw-bold fs-5 text-success">{{ number_format($totalDeposit, 2) }}</span>
                                </div>
                            </div>
                        </article>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <article class="card dg-card h-100">
                            <div class="card-body dg-card-body py-2">
                                <div class="dg-summary-item mb-0 border-0 p-0">
                                    <span class="dg-label d-block mb-1">Total Withdraw</span>
                                    <span class="fw-bold fs-5 text-danger">{{ number_format($totalWithdraw, 2) }}</span>
                                </div>
                            </div>
                        </article>
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <article class="card dg-card h-100">
                            <div class="card-body dg-card-body py-2">
                                <div class="dg-summary-item mb-0 border-0 p-0">
                                    <span class="dg-label d-block mb-1">Net Balance</span>
                                    <span class="fw-bold fs-5">{{ number_format($netBalance, 2) }}</span>
                                </div>
                            </div>
                        </article>
                    </div>
            ...
            <section class="dg-section dg-filter @if (request('print')) d-print-none @endif">
                <article class="card dg-card">
                    ...
                    <div class="card-body dg-card-body dg-filter-card-body">
                        <form method="GET" action="{{ route('company.loan-saving-ledger.index') }}" class="dg-filter-form">
                            <div class="dg-filter-grid">
                                ...
                                <div class="dg-filter-field dg-filter-field-status">
                                    <label for="type" class="dg-filter-label">Type</label>
                                    <select name="type" id="type" class="form-select dg-select dg-filter-control">
                                ...
                                <div class="dg-filter-actions">
                                    ...
                                    <a href="{{ route('company.loan-saving-ledger.index') }}" class="btn btn-outline-secondary dg-btn">Reset</a>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <section class="dg-section">
                <article class="card dg-card">
                    <header class="card-header dg-card-header dg-list-card-header @if (request('print')) d-print-none @endif">
                        <h2 class="dg-list-card-title">Loan Saving Ledger List</h2>

                        <form method="GET" action="{{ route('company.loan-saving-ledger.index') }}" class="dg-list-per-page">
                    ...
                    <div class="card-body dg-card-body dg-list-card-body">
                        <div class="table-responsive">
                            <table class="table dg-table" id="loanSavingLedgerTable">
                                <thead class="dg-head">
                                    <tr>
                                        <th scope="col">#</th>
                                ...
                                <tbody class="dg-body">
                                    @forelse ($ledgers as $ledger)
                                        <tr class="dg-row">
                                            ...
                                            <td class="text-end">{{ number_format($ledger->balance_after, 2) }}</td>
                                            <td>{{ \Illuminate\Support\Str::limit($ledger->note ?? '-', 40) }}</td>
                                            <td>{{ $ledger->createdBy->name ?? '-' }}</td>
                                            <td class="dg-action-col d-print-none">
                                                @if ($canView)
                                                    <div class="dg-action-group" role="group" aria-label="Saving ledger actions">
                                                        <a href="{{ route('company.loan-saving-ledger.show', $ledger->id) }}" class="btn btn-sm btn-outline-primary dg-action-btn">View</a>
                                                    </div>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
                                        <tr class="dg-row">
                                            <td colspan="12" class="text-center">No saving ledger entries found</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3 flex-wrap gap-2 @if (request('print')) d-print-none @endif">
                            <p class="mb-0 text-muted">
                                Showing {{ $ledgers->firstItem() ?? 0 }} to {{ $ledgers->lastItem() ?? 0 }} of {{ $ledgers->total() }} records
                            </p>

                            <nav aria-label="Loan saving ledger pagination">
                                {{ $ledgers->links() }}
                            </nav>
                        </div>
                    </div>
...
<script>
(function () {
    const exportBtn = document.getElementById('loanSavingLedgerExportBtn');
    const table = document.getElementById('loanSavingLedgerTable');
    ...
    exportBtn.addEventListener('click', function () {
        ...
        link.href = url;
        link.download = 'loan-saving-ledger-' + new Date().toISOString().slice(0, 10) + '.csv';
