...
<div class="dg-page">
    <div class="dg-page-header">
        ...
        <a href="{{ route('company.register') }}" class="btn btn-primary dg-btn">Create Registration</a>
    </div>
        ...
        <div class="dg-card-body card-body">
            <div class="dg-table-scroll dg-table-wrap">
                <table class="table table-hover align-middle dg-table">
                    <thead class="dg-head"><tr><th scope="col">Company</th><th scope="col">Country</th><th scope="col">Registration Staff</th><th scope="col">Applicant</th><th scope="col" class="dg-col-date">Submitted</th><th scope="col" class="dg-col-status">Status</th><th scope="col" class="dg-action-col">Actions</th></tr></thead>
                    <tbody class="dg-body">
                        @forelse($registrations as $r)
                            <tr class="dg-row">
                                <td><strong>{{ $r->company_name }}</strong></td>
                                <td>{{ $r->countryMaster?->name ?? 'N/A' }}</td>
                                <td>{{ $r->registeredBy?->name ?? 'Historical / unavailable' }}{{ $r->registeredBy ? ' (User #'.$r->registeredBy->id.')' : '' }}</td>
                                <td>{{ $r->full_name }}</td>
                                ...
                                <td class="dg-action-col">
                                    ...
                                    @if($r->status === 'pending' && app(\App\Services\PlatformAuthorizationService::class)->can(auth()->user(), 'platform_registrations_approve'))
                        ...
                        @empty
                            <tr><td colspan="7"><div class="dg-empty-state">No company registrations are available.</div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
