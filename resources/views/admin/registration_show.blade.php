...
<div class="dg-page dg-record-print">
    ...
    <article id="printArea" class="dg-card card dg-record-sheet">
        ...
        <div class="dg-record-grid">
            ...
            <section class="dg-record-section" aria-labelledby="registration-record-title"><h2 id="registration-record-title" class="dg-record-section-title">Record Information</h2><dl class="dg-record-list"><div><dt>Registration Staff</dt><dd>{{ $registration->registeredBy?->name ?? 'Historical / unavailable' }}{{ $registration->registeredBy ? ' (User #'.$registration->registeredBy->id.')' : '' }}</dd></div><div><dt>Submitted</dt><dd>{{ $registration->created_at?->format('Y-m-d H:i') ?? 'N/A' }}</dd></div><div><dt>Approved By</dt><dd>{{ $registration->approver?->name ?? 'N/A' }}</dd></div><div><dt>Approved At</dt><dd>{{ $registration->approved_at?->format('Y-m-d H:i') ?? 'N/A' }}</dd></div><div><dt>Rejected By</dt><dd>{{ $registration->rejector?->name ?? 'N/A' }}</dd></div><div><dt>Rejected At</dt><dd>{{ $registration->rejected_at?->format('Y-m-d H:i') ?? 'N/A' }}</dd></div></dl></section>
        </div>
